...
class InventoryController {
    ...
    async stockIn(req, res) {
        try {
            ...
            const parseResult = inventory_validation_1.stockInSchema.safeParse(req.body);
            ...
            logger_1.logger.error('Failed to stock in', {
    ...
    async stockOut(req, res) {
        try {
            const tenantId = req.tenant?.tenantId || 'tenant1'; // Default for testing
            const parseResult = inventory_validation_1.stockOutSchema.safeParse(req.body);
            if (!parseResult.success) {
                res.status(400).json({ errors: parseResult.error.issues });
                return;
            }
            ...
            logger_1.logger.error('Failed to stock out', {
                error: error instanceof Error ? { message: error.message, stack: error.stack } : error,
                tenantId: req.tenant?.tenantId,
                stockOut: req.body
            });
    ...
    async getByProductId(req, res) {
        try {
            ...
            logger_1.logger.error('Failed to get inventory by product ID', { error, tenantId: req.tenant?.tenantId, productId: req.params.productId });
    ...
    async list(req, res) {
        try {
            ...
            logger_1.logger.error('Failed to list inventory', { error, tenantId: req.tenant?.tenantId });
    ...
    async adjustStock(req, res) {
        try {
            const tenantId = req.tenant?.tenantId || 'tenant1'; // Default for testing
            const parseResult = inventory_validation_1.stockAdjustmentSchema.safeParse(req.body);
            if (!parseResult.success) {
                res.status(400).json({ errors: parseResult.error.issues });
                return;
            }
