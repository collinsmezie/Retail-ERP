...
const productSchema = z.object({
  ...
  price: z.number({ invalid_type_error: 'Enter a valid number' }).min(0, 'Price must be at least 0'),
  cost: z.number({ invalid_type_error: 'Enter a valid number' }).min(0, 'Cost must be at least 0').optional(),
      ...
      <Box component="form" onSubmit={handleSubmit(onSubmit)}>
        <Stack spacing={1.5}>
          ...
          <Stack direction="row" spacing={1}>
            <Button type="submit" variant="contained" size="small" disabled={mutation.status === 'pending'}>
              {mutation.status === 'pending' ? 'Saving…' : 'Create Product'}
            </Button>
